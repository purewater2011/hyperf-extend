...
class Connection implements ConnectionInterface
{
    ...
    {
        if ($this->isMysqlForbidden($query)) {
            return [];
        }
     ...
     * 该sql语句是否被熔断.
     */
    private function isMysqlForbidden(string $query): bool
    {
        $is_current_query_forbidden = false;
        $mysql_forbidden_arr = config('mysql_forbidden', []);
        foreach ($mysql_forbidden_arr as $pattern_item) {
            if (! preg_match("/{$pattern_item}/", $query)) {
                continue;
            }
            $is_current_query_forbidden = true;
            break;
        }
        return $is_current_query_forbidden;
    }
}
